...
$extra = ':00';
													...
													// SERVER TIME
					                                  ...
					                                  $secs = strtotime($newsearchtime)-strtotime("00:00:00");
                                                   
												   // UPCOING SERVER TIME
												   
												      $timese = date("H:i:s",strtotime($timess)+$secs);
...
if ($city_to == 'ok') {
 ...
 ,tblstation.bus_code,tblbus.`route`,tblstation.arrival AS starttime,tblbus.`city_from`,tblbus.vendor,tblbus.`city_to`,tblbus.`seat`,tblstation.`day` AS startday,tblstation.distance FROM tblstation,tblbus INNER JOIN `tblbustype` ON tblbustype.`bus_code` = tblbus.`bus_code` WHERE tblstation.`station` ='$city_from' AND tblstation.`bus_code`= tblbus.`bus_code` AND STATUS ='active'  AND tblstation.arrival BETWEEN '$timess' AND '$timese' ) AS TAB_1, 
    ...
    $buss = $mysqli->query("
											 SELECT *   FROM (SELECT tblstation.id
 ,tblstation.bus_code,tblbus.`route`,tblstation.arrival AS starttime,tblbus.`city_from`,tblbus.vendor,tblbus.`city_to`,tblbus.`seat`,tblstation.`day` AS startday,tblstation.distance FROM tblstation,tblbus INNER JOIN `tblbustype` ON tblbustype.`bus_code` = tblbus.`bus_code` WHERE tblstation.`station` ='$city_from' AND tblstation.`bus_code`= tblbus.`bus_code` AND STATUS ='active'  AND tblstation.arrival BETWEEN '$timess' AND '$timese' ) AS TAB_1, (SELECT   bus_code,arrival AS endtime ,tblstation.`day` AS endday,tblstation.distance FROM  tblstation ) AS TAB_2
												WHERE (TAB_1.distance < TAB_2.distance && TAB_1.bus_code = TAB_2.bus_code)
												
												
												GROUP BY TAB_1.`bus_code`
												ORDER BY starttime ASC LIMIT $limit, 20
 ...
 ,tblstation.bus_code,tblbus.`route`,tblstation.arrival AS starttime,tblbus.`city_from`,tblbus.vendor,tblbus.`city_to`,tblbus.`seat`,tblstation.`day` AS startday,tblstation.distance FROM tblstation,tblbus INNER JOIN `tblbustype` ON tblbustype.`bus_code` = tblbus.`bus_code` WHERE tblstation.`station` ='$city_from' AND tblstation.`bus_code`= tblbus.`bus_code` AND STATUS ='active'  AND tblstation.arrival BETWEEN '$timess' AND '$timese' ) AS TAB_1, 
												(SELECT   bus_code,arrival AS endtime ,tblstation.`day` AS endday,tblstation.distance
												FROM  tblstation 
												WHERE tblstation.`station` ='$city_to'
												) AS TAB_2
												WHERE (TAB_1.distance < TAB_2.distance && TAB_1.bus_code = TAB_2.bus_code)
												GROUP BY TAB_1.`bus_code`
												ORDER BY starttime ASC ");
